    ...
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col justify-between hidden md:flex shrink-0">
        <div class="flex-1 overflow-y-auto">
            ...
            <nav class="px-4 space-y-1 mb-4">
                ...
                <a href="{{ route('lecturer.materials.create') }}" class="flex items-center space-x-3 px-4 py-3 bg-amber-500 text-white rounded-lg font-medium text-sm">
        ...
        <div class="flex-1 overflow-y-auto p-8">
            <div class="max-w-4xl mx-auto">
                <div class="mb-8 animate-fade-in-up">
                    <h1 class="text-3xl font-bold text-slate-800 mb-2">Tulis Materi Baru</h1>
                    <p class="text-slate-500">Buat materi lengkap dengan video dan contoh kode biar mahasiswa paham.</p>
                </div>

                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-8 animate-fade-in-up">
                    <form action="{{ route('lecturer.materials.store') }}" method="POST">
                        @csrf
                        
                        <div class="mb-5">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Judul Materi</label>
                            <input type="text" name="title" required class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors" placeholder="Contoh: Pengenalan Array Multi-dimensi">
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select name="level" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white transition-colors cursor-pointer">
                                <option value="Pemula">Level Pemula (Basic)</option>
                                <option value="Menengah">Level Menengah (Intermediate)</option>
                                <option value="Lanjutan">Level Lanjutan (Advanced)</option>
                            </select>
                        </div>

                        <div class="mb-5">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Link Video Penjelasan (YouTube dll)</label>
                            <input type="url" name="video_url" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors text-blue-600" placeholder="https://www.youtube.com/watch?v=...">
                        </div>

                        <div class="mb-5">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Visualisasi Kode Program (Opsional)</label>
                            <textarea name="code_visualization" rows="8" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors bg-slate-900 text-green-400 font-mono text-sm leading-relaxed" placeholder="// Ketik kode C++ atau bahasa lain di sini..."></textarea>
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Konten / Penjelasan Modul</label>
                            <textarea name="content" required rows="10" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors" placeholder="Ketik teori dan materi panjang lebar di sini."></textarea>
                        </div>

                        <div class="flex items-center mb-8 bg-slate-50 p-4 rounded-lg border border-slate-100">
                            <input type="checkbox" id="is_published" name="is_published" value="1" checked class="w-5 h-5 text-amber-500 bg-white border-slate-300 rounded focus:ring-amber-500 cursor-pointer">
                            <label for="is_published" class="ml-3 text-sm font-bold text-slate-700 cursor-pointer">Langsung Terbitkan (Publish)</label>
                        </div>
