            ...
            <div class="max-w-4xl mx-auto">
                <div class="mb-8 animate-fade-in-up">
                    <div class="flex items-center space-x-3 mb-2">
                        <a href="{{ route('lecturer.dashboard') }}" class="text-slate-400 hover:text-amber-500 transition-colors">
                            <i class="fa-solid fa-circle-left text-2xl"></i>
                        </a>
                        <h1 class="text-3xl font-black text-slate-800 uppercase tracking-tight">Edit Quest</h1>
                    </div>
                    <p class="text-slate-500 font-medium text-sm">Perbarui ilmu atau ralat misi yang ada.</p>
                </div>

                <div class="bg-white rounded-2xl border-2 border-slate-200 shadow-sm p-8">
                    <form action="{{ route('lecturer.materials.update', $material->id) }}" method="POST" id="quest-form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest">Judul Misi / Materi</label>
                                <input type="text" name="title" value="{{ $material->title }}" required class="w-full px-5 py-4 rounded-xl border-2 border-slate-100 focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-50 text-slate-800 font-bold transition-all">
                            </div>

                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest">Target Kasta</label>
                        ...
                        <div class="mb-8">
                            <label class="block text-xs font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest">Ringkasan Misi (Poin Penting)</label>
                            ...
                            <div class="border-2 border-slate-100 rounded-xl overflow-hidden shadow-inner">
                                ...
                                <div id="editor-container" class="bg-white min-h-[300px] text-slate-700 font-medium px-4 py-2">
                                    {!! $material->content !!}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8 bg-blue-50 border-2 border-dashed border-blue-300 rounded-2xl p-8 text-center hover:bg-blue-100 transition-colors group relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                ...
                                <h3 class="font-black text-slate-700 mb-1 text-lg">Update Gulungan Asli (PDF)</h3>
                                @if($material->pdf_path)
                                    <p class="text-[10px] text-emerald-600 font-black uppercase tracking-widest mb-4">
                                        <i class="fa-solid fa-circle-check"></i> File Terdeteksi: {{ basename($material->pdf_path) }}
                                    </p>
                                @else
                                    <p class="text-xs text-slate-500 mb-6 font-bold uppercase tracking-wider italic">Belum ada file terlampir</p>
                                @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase mb-2 ml-1">Video URL</label>
                                <input type="url" name="video_url" value="{{ $material->video_url }}" class="w-full px-4 py-4 rounded-xl border-2 border-slate-100 focus:border-red-500 text-sm font-medium transition-all" placeholder="https://youtube.com/...">
                            </div>
                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase mb-2 ml-1">Quick Code Snippet</label>
                                <input type="text" name="code_visualization" value="{{ $material->code_visualization }}" class="w-full px-4 py-4 rounded-xl border-2 border-slate-100 focus:border-emerald-500 font-mono text-xs transition-all" placeholder="int x = 10;">
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 mb-8">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Terbitkan langsung ke Player</p>
                                </div>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_published" value="1" {{ $material->is_published ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <div class="flex justify-end space-x-4">
                            <button type="submit" class="w-full md:w-auto bg-emerald-500 hover:bg-emerald-600 text-white font-black py-4 px-12 rounded-xl shadow-[0_6px_0_0_#059669] hover:shadow-[0_2px_0_0_#059669] hover:translate-y-[4px] transition-all flex items-center justify-center space-x-3">
    ...
    <div x-show="showLevelModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-3xl w-full max-w-lg p-8 relative z-10 shadow-2xl animate-pop-in" @click.stop>
            <div class="text-center mb-8">
                ...
                <h3 class="text-2xl font-black text-slate-800">Tentukan Kasta Misi</h3>
                <p class="text-slate-500 text-sm font-medium italic">Status saat ini: <span class="font-bold text-amber-600 uppercase" x-text="selectedLevel"></span></p>
            </div>
            ...
            <div class="space-y-4">
                <button @click="selectedLevel = 'Pemula'; showLevelModal = false" class="w-full p-5 rounded-2xl border-2 flex items-center justify-between group transition-all" :class="selectedLevel === 'Pemula' ? 'border-slate-400 bg-slate-50 shadow-inner' : 'border-slate-100 hover:border-amber-500 hover:bg-amber-50'">
                    <div class="flex items-center space-x-4 text-left">
                        ...
                        <div><p class="font-black text-slate-800 uppercase text-sm tracking-wide">Pemula (Basic)</p></div>
                    </div>
                ...
                <button @click="selectedLevel = 'Menengah'; showLevelModal = false" class="w-full p-5 rounded-2xl border-2 flex items-center justify-between group transition-all" :class="selectedLevel === 'Menengah' ? 'border-blue-400 bg-blue-50 shadow-inner' : 'border-slate-100 hover:border-blue-500 hover:bg-blue-50'">
                    <div class="flex items-center space-x-4 text-left">
                        ...
                        <div><p class="font-black text-blue-800 uppercase text-sm tracking-wide">Menengah (Intermediate)</p></div>
                    </div>
                ...
                <button @click="selectedLevel = 'Lanjutan'; showLevelModal = false" class="w-full p-5 rounded-2xl border-2 flex items-center justify-between group transition-all" :class="selectedLevel === 'Lanjutan' ? 'border-emerald-400 bg-emerald-50 shadow-inner' : 'border-slate-100 hover:border-emerald-500 hover:bg-emerald-50'">
                    <div class="flex items-center space-x-4 text-left">
                        ...
                        <div><p class="font-black text-emerald-800 uppercase text-sm tracking-wide">Lanjutan (Advanced)</p></div>
                    </div>
