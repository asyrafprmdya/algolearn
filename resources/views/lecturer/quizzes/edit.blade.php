...
<div class="flex h-screen bg-slate-50 overflow-hidden">
    @php
        ...
        $mappedQuestions = $quiz->questions->map(function($q) {
            return [
                'id' => $q->id,
                'text' => $q->question_text,
                'type' => $q->type ?? 'multiple_choice',
                'a' => $q->option_a !== '-' ? $q->option_a : '',
                'b' => $q->option_b !== '-' ? $q->option_b : '',
                'c' => $q->option_c !== '-' ? $q->option_c : '',
            ...
            <div class="max-w-4xl mx-auto animate-fade-in-up">
                ...
                <div class="mb-8">
                    <div class="flex items-center space-x-3 mb-2">
                        ...
                        <span class="bg-amber-100 text-amber-700 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-sm">Edit Kuis</span>
                    </div>
                    <h1 class="text-3xl font-black text-slate-800 tracking-tight uppercase">Update Kuis: {{ $material->title }}</h1>
                    ...
                    <div class="bg-white rounded-3xl border-2 border-slate-200 shadow-sm mb-8 relative group hover:border-amber-300 transition-colors z-20">
                        ...
                        <div class="relative z-10 p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" value="{{ $quiz->title }}" required class="w-full px-5 py-4 rounded-xl border-2 border-slate-100 focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-50 text-slate-800 font-bold transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase mb-2 tracking-widest ml-1">Nilai Kelulusan (KKM)</label>
                                <div class="relative">
                                    <input type="number" name="passing_grade" value="{{ $quiz->passing_grade }}" required class="w-full px-5 py-4 pl-14 rounded-xl border-2 border-slate-100 focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50 text-slate-800 font-black text-lg transition-all">
